...
// BlendMode lives in engine/core/Types.h now (Phase 3 first-half): building
// GraphicsDevice::GetOrCreatePipeline's blend-state variants requires the
// backend to know about it too, and GraphicsDevice must not depend on this
// (higher) layer, so the enum moved down rather than being duplicated here.
