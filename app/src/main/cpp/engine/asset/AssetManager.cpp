...
AssetType AssetManager::DetectAssetType(const std::string& path) const {
    ...
    if (memcmp(header, "\x00\x00\x00\x20" "ftypglTF", 12) == 0 || memcmp(header, "glTF", 4) == 0) return AssetType::Model;
...
bool AssetManager::SaveDatabase(const std::string& filePath) {
    ...
    for (const auto& [id, ptr] : assets_) {
        ...
        first = false;
        file << "    {\n"
             << "      \"id\": \"" << EscapeJsonString(ptr->assetId) << "\",\n"
             << "      \"sourcePath\": \"" << EscapeJsonString(ptr->sourcePath) << "\",\n"
             << "      \"type\": " << static_cast<int>(ptr->type) << ",\n"
             << "      \"displayName\": \"" << EscapeJsonString(ptr->displayName) << "\",\n"
             << "      \"fileSizeBytes\": " << ptr->fileSizeBytes << ",\n"
             << "      \"width\": " << ptr->width << ",\n"
             << "      \"height\": " << ptr->height << ",\n"
             << "      \"frameCount\": " << ptr->frameCount << ",\n"
             << "      \"frameRate\": " << ptr->frameRate << ",\n"
             << "      \"durationUs\": " << ptr->durationUs << ",\n"
             << "      \"proxyStatus\": " << static_cast<int>(ptr->proxyStatus) << ",\n"
             << "      \"proxyPath\": \"" << EscapeJsonString(ptr->proxyPath) << "\",\n"
             << "      \"proxyWidth\": " << ptr->proxyWidth << ",\n"
             << "      \"proxyHeight\": " << ptr->proxyHeight << ",\n"
             << "      \"isMissing\": " << (ptr->isMissing ? "true" : "false") << "\n"
             << "    }";
    }
    file << "\n  ]\n}\n";
    return true;
