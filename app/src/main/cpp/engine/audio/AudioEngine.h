...
public:
    ...
    bool Open(std::string_view path, std::string* error = nullptr);
    bool OpenFromMediaExtractor(AMediaExtractor* extractor, int trackIndex, std::string* error = nullptr);
    void Close();
...
private:
    ...
    std::atomic<bool> analysisStopFlag_{false};
