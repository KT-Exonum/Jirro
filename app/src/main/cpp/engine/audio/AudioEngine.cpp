...
bool AudioDecoder::Open(std::string_view path, std::string* error) {
    extractor_ = AMediaExtractor_new();
    if (!extractor_) {
        if (error) *error = "Failed to create media extractor";
        return false;
    }
    ...
    if (status != AMEDIA_OK) {
        ...
        Close();
        if (error) *error = "Failed to set data source";
        return false;
    }
    ...
    for (int i = 0; i < trackCount; ++i) {
        ...
        if (mime && strncmp(mime, "audio/", 6) == 0) {
            ...
            if (!codec_) {
                ...
                Close();
                if (error) *error = "Failed to create decoder";
                return false;
            }
            ...
            if (cs != AMEDIA_OK) {
                ...
                AMediaFormat_delete(fmt);
                Close();
                if (error) *error = "Failed to configure codec";
                return false;
            }
    ...
    Close();
    if (error) *error = "No audio track found";
    return false;
}

bool AudioDecoder::OpenFromMediaExtractor(AMediaExtractor* extractor, int trackIndex, std::string* error) {
...
AudioEngine::~AudioEngine() {
    analysisStopFlag_.store(true, std::memory_order_release);
...
std::string AudioEngine::LoadAudio(std::string_view path) {
    ...
    std::string err;
    if (!decoder->Open(path, &err)) {
        LOGE("Failed to load audio: %s", err.c_str());
        return {};
    }

    std::stringstream ss;
    ss << "audio_" << std::hex << std::hash<std::string>{}(std::string(path));
    std::string audioId = ss.str();
...
AudioClip* AudioEngine::CreateClip(const std::string& audioId, int64_t startUs, int64_t endUs) {
    ...
    std::stringstream ss;
    ss << "clip_" << nextClipId_.fetch_add(1);
    std::string clipId = ss.str();
...
SpectrumData AudioEngine::GetCurrentSpectrum() {
    ...
    return analyzer_->AnalyzeSpectrum(mixed, mixer_->GetChannels());
}
...
BeatInfo AudioEngine::GetCurrentBeatInfo() {
    ...
    auto beatInfo = analyzer_->DetectBeats(mixed, mixer_->GetChannels(), currentTimeSec_);
...
std::vector<float> AudioEngine::GetCurrentWaveform(size_t points) {
    ...
    return analyzer_->GetWaveform(mixed, mixer_->GetChannels(), points);
}
...
void AudioEngine::AnalysisThreadMain() {
    while (!analysisStopFlag_.load(std::memory_order_acquire)) {
        std::unique_lock<std::mutex> lock(analysisMutex_);
        analysisCV_.wait_for(lock, std::chrono::milliseconds(50), [this] { return analysisStopFlag_.load(std::memory_order_acquire); });
    }
}
