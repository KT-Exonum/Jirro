...
bool VideoDecoder::Open(GraphicsDevice& device) {
    ...
    for (int i = 0; i < trackCount; ++i) {
        ...
        if (mime && strncmp(mime, "video/", 6) == 0) {
            ...
            media_status_t status = AImageReader_new(width_, height_, AIMAGE_FORMAT_YUV_420_888, 4, &imageReader_);
            ...
            media_status_t ws = AImageReader_getWindow(imageReader_, &surface);
            if (ws != AMEDIA_OK || !surface) {
                LOGE("Failed to get ANativeWindow from AImageReader: %d", ws);
                AMediaFormat_delete(fmt);
                Close();
                return false;
            }
        ...
        if (ptsUs > 0) {
            ...
            if (status == AMEDIA_OK && image) {
                ...
                if (status == AMEDIA_OK && hardwareBuffer) {
                    GraphicsDevice::HardwareBufferHandle hbHandle;
                    hbHandle.nativeHandle = hardwareBuffer;
                    auto result = device.ImportHardwareBuffer(hbHandle, width_, height_);
                    if (result) {
                        DecodedFrame frame;
...
void FrameCache::EvictOutsideWindow() {
    ...
    double windowStart = playheadHint_ - config_.framesBehind / config_.frameRate;
    double windowEnd = playheadHint_ + config_.framesAhead / config_.frameRate;
