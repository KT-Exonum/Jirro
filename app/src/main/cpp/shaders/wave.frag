#version 450

layout(set = 0, binding = 0) uniform Uniforms {
    mat4 uProjection;
    mat4 uView;
    float uTime;
    float uResolution[2];
    int uFrameIndex;
} ubo;

layout(set = 0, binding = 1) uniform sampler2D uInputTexture;
layout(set = 0, binding = 2) uniform sampler2D uInputTexture2;

layout(location = 0) in vec2 fTexCoord;
layout(location = 1) in vec2 fCenterDist;
layout(location = 0) out vec4 outColor;

layout(push_constant) uniform PushConstants {
    float uTransform[9];
    float uFrequency;
    float uMagnitude;
    float uAngle;
    float uPhase;
    int uWaveType;
    float uDecay;
    float uRotation;
    float uSeed;
    float uAmount;
    float uSpeed;
    float uScale;
    float uOctaves;
    float uIntensity;
    float uBlockSize;
    float uChromatic;
    float uNoiseAmount;
    float uScanlineAmount;
    float uDistortion;
    float uColorBleed;
    float uJitter;
} pc;

float triangleWave(float x) {
    return abs(fract(x + 0.5) * 2.0 - 1.0);
}

void main() {
    vec2 uv = fTexCoord;
    
    // Wave distortion along angle direction
    float angleRad = radians(pc.uAngle);
    float dir = sin(angleRad) * uv.x + cos(angleRad) * uv.y;
    float time = ubo.uTime * pc.uSpeed;
    
    float wave = sin(dir * pc.uFrequency * 50.0 - time * 10.0 + pc.uPhase * 0.01) * pc.uMagnitude * pc.uAmount;
    
    // Displace perpendicular to wave direction
    vec2 perp = vec2(-sin(angleRad), cos(angleRad));
    uv += perp * wave;
    uv = clamp(uv, vec2(0.0), vec2(1.0));
    
    outColor = texture(uInputTexture, uv);
}