...
void main() {
    ...
    float ripple = sin(dist * pc.uFrequency * 50.0 - time * 10.0) * pc.uMagnitude * pc.uAmount;
    
    float newDist = dist + ripple;
    vec2 newUV = center + vec2(cos(angle), sin(angle)) * newDist;
