...
void main() {
    ...
    // Twist distortion - rotation increases with distance from center
    float time = ubo.uTime * pc.uSpeed;
    float twistAmount = (pc.uFrequency + sin(time * pc.uSpeed) * pc.uMagnitude) * pc.uAmount;
    float twist = dist * twistAmount * 10.0;
