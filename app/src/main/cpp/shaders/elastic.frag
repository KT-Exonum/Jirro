...
void main() {
    ...
    // Elastic spring motion with overshoot
    float phase = ubo.uTime * pc.uFrequency * 6.2831853 + pc.uPhase * 0.01;
    float decay = exp(-ubo.uTime * pc.uDecay);
    
    // Elastic uses exponentially decaying sine
    float elastic = sin(phase) * pow(2.0, -phase * pc.uDecay * 0.5) * pc.uMagnitude;
    
    // Apply scale oscillation
    float scale = 1.0 + elastic * 0.001;
    uv *= scale;
