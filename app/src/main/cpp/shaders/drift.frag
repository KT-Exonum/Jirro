...
void main() {
    ...
    // Slow drift using low-frequency noise
    float time = ubo.uTime * pc.uFrequency;
    vec2 drift = vec2(
        (fbm(vec2(time, float(pc.uSeed) * 0.01), int(pc.uOctaves)) - 0.5) * 2.0,
        (fbm(vec2(time + 100.0, float(pc.uSeed) * 0.01), int(pc.uOctaves)) - 0.5) * 2.0
    ) * pc.uMagnitude * pc.uAmount * 0.02;
