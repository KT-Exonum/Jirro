...
void main() {
    ...
    // Mesh warp using grid-based displacement
    float gridSize = 1.0 / pc.uScale;
    vec2 grid = uv * pc.uScale;
    vec2 cell = floor(grid);
    vec2 cellUV = fract(grid);
    
    // Get displacement for each grid cell
    float time = ubo.uTime * pc.uSpeed;
    vec2 disp = vec2(
        (noise(cell + vec2(time, float(pc.uSeed))) - 0.5) * 2.0,
        (noise(cell + vec2(time + 100.0, float(pc.uSeed))) - 0.5) * 2.0
    ) * pc.uAmount * pc.uIntensity * 0.02;
