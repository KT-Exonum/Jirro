...
void Engine::ThreadMain() {
    ...
    while (running_.load(std::memory_order_acquire) && !engineStopFlag_.load()) {
        Tick();
...
void Engine::StartExport(ExportCommand&& cmd) {
    ...
    config.duration = cmd.endTime - cmd.startTime;
    config.bitrateMbps = cmd.bitrateMbps;

    auto completionCb = std::move(cmd.onComplete);
    exportPipeline_->StartExport(config,
        [](const vfx::ExportProgress& progress) {
            LOGI("Export progress: %.1f%% - %s", progress.progress * 100, progress.currentOperation.c_str());
        });
    if (completionCb) {
        // In real implementation, would be called asynchronously when export finishes
        // For now, call immediately with pending status
        vfx::ExportResult result;
        result.success = false;
        result.errorMessage = "Export queued";
        completionCb(result);
    }
}
...
Engine::LowEndSettings Engine::GetRecommendedLowEndSettings() {
    ...
    settings.maxTextureSize = static_cast<int>(caps.maxTextureDimension);
